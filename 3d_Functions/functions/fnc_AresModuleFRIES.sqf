...
// Start script
// Check if selected object(s) are helicopter and message zeus operator if no object is helicopter
private _count = "Helicopter" countType _selectedObjects;

if (_count < 1) exitWith
{
   ["No helicopter(s) selected"] call Achilles_fnc_showZeusErrorMessage;
};

// Remove any none helicopter objects from selection
_selectedHelis = [];
{
	if (_x isKindOf "Helicopter") then {_selectedHelis pushBack _x};
} forEach _selectedObjects;

// Add FRIES capability to selected object(s)
{
	[_x] call ace_fastroping_fnc_equipFRIES;
} forEach _selectedHelis;

//Message zeus operator with changed objects
["Selected objects: %1, Equipped with FRIES", _selectedHelis] call Achilles_fnc_showZeusErrorMessage;
